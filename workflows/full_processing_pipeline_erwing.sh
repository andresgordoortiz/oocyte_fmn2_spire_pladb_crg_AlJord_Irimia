...
# This script processes the Erwing Sarcoma data by performing the following steps:
# 1. Align reads
...
jid2=$(sbatch --dependency=afterok:$jid1 $PWD/scripts/bash/erwing_sarcoma/fastqc_multiqc_erwing.sh | tr -cd '[:digit:].')
echo "...second job ID is $jid2"

echo "Submitting third job: Align reads..."
jid3=$(sbatch --dependency=afterok:$jid2 $PWD/scripts/bash/erwing_sarcoma/vast_align_erwing.sh $VASTDB_PATH | tr -cd '[:digit:].')
echo "...third job ID is $jid3"

echo "Submitting fourth job: Run vast combine..."
jid4=$(sbatch --dependency=afterok:$jid3 $PWD/scripts/bash/erwing_sarcoma/vast_combine_erwing.sh $VASTDB_PATH | tr -cd '[:digit:].')
echo "...fifth job ID is $jid4"

echo "Submitting fifth job: Multiqc..."
jid5=$(sbatch --dependency=afterok:$jid2 $PWD/scripts/bash/erwing_sarcoma/multiqc.sh | tr -cd '[:digit:].')
